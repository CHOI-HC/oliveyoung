...
def ver1_1(request):
    ...
    c1_score = (float((cat_df.loc[category,'군집1']))/cat_df.loc[:,'군집1'].values.sum())*(skin_df1.loc[skinconcern,'군집1'].sum()*0.5+float(skin_df1.loc[skintype,'군집1']))
    c2_score = (float((cat_df.loc[category,'군집2']))/cat_df.loc[:,'군집2'].values.sum())*(skin_df1.loc[skinconcern,'군집2'].sum()*0.5+float(skin_df1.loc[skintype,'군집2']))
    c3_score = (float((cat_df.loc[category,'군집3']))/cat_df.loc[:,'군집3'].values.sum())*(skin_df1.loc[skinconcern,'군집3'].sum()*0.5+float(skin_df1.loc[skintype,'군집3']))
    c4_score = (float((cat_df.loc[category,'군집4']))/cat_df.loc[:,'군집4'].values.sum())*(skin_df1.loc[skinconcern,'군집4'].sum()*0.5+float(skin_df1.loc[skintype,'군집4']))
    c5_score = (float((cat_df.loc[category,'군집5']))/cat_df.loc[:,'군집5'].values.sum())*(skin_df1.loc[skinconcern,'군집5'].sum()*0.5+float(skin_df1.loc[skintype,'군집5']))
...
def ver1_result(request):
    ...
    ##추가 (바디케어의 경우 피부타입 만족도 데이터가 없음, 모두 0이므로 에러가 나는 문제->랜덤값으로 채움)
    ...
    p1_kw_fr=p1_kw_fr.tolist()
    p1_1_kw_fr=round(p1_kw_fr[0]*100)
    p1_2_kw_fr=round(p1_kw_fr[1]*100)
    p2_kw_fr=total_keywords_df.loc[i[1],keywords2].values
    p2_kw_fr=p2_kw_fr/p_rev_cnt[1]
    p2_kw_fr=p2_kw_fr.tolist()
    p2_1_kw_fr=round(p2_kw_fr[0]*100)
    p2_2_kw_fr=round(p2_kw_fr[1]*100)
    p3_kw_fr=total_keywords_df.loc[i[2],keywords2].values
    p3_kw_fr=p3_kw_fr/p_rev_cnt[2]
    p3_kw_fr=p3_kw_fr.tolist()
    p3_1_kw_fr=round(p3_kw_fr[0]*100)
    p3_2_kw_fr=round(p3_kw_fr[1]*100)
    ...
    p_kw_fr=[p1_1_kw_fr,p1_2_kw_fr,p2_1_kw_fr,p2_2_kw_fr,p3_1_kw_fr,p3_2_kw_fr]
    ...
    if p_con_rate1<0.5:
     p_con_rate1=(p_con_rate1+0.5)*100
    else: p_con_rate1=p_con_rate1*100

    p_con_rate2=skin_df1.loc[skinconcern2,topic_num2]
    if p_con_rate2<0.4:
     p_con_rate2=(p_con_rate2+0.6)*100
    else: p_con_rate2=p_con_rate2*100
    p_con_rate=['%d'%p_con_rate1,'%d'%p_con_rate2]

    # 제품별 그래프 수치 리스트
    p1_gr_value=[]
    p2_gr_value=[]
    p3_gr_value=[]
    p1_gr_value.extend([p1_skin_rate,p_con_rate1/100,p_con_rate2/100,p1_1_kw_fr/100,p1_2_kw_fr/100])
    p2_gr_value.extend([p2_skin_rate,p_con_rate1/100,p_con_rate2/100,p2_1_kw_fr/100,p2_2_kw_fr/100])
    p3_gr_value.extend([p3_skin_rate,p_con_rate1/100,p_con_rate2/100,p3_1_kw_fr/100,p3_2_kw_fr/100])
